...
class PageBackground {
  ...
  private initBackground = () => {
    ...
    const randomLetters = this.getRandomAmountFromArray<LetterPosition>(this.letterPositions, Number.parseInt((lines * 0.75).toFixed()));
    ...
    for (const letter of this.letterInstances) {
      ...
      if (alpha <= 0 && Date.now() > letter.fadeout) {
        ...
        this.letterInstances.push({
          ...
          fadeout:
            Date.now() +
            (this.LETTER_FADE_DURATION[0] + Math.random() * (this.LETTER_FADE_DURATION[1] - this.LETTER_FADE_DURATION[0])) * 1000,
        });
      }
